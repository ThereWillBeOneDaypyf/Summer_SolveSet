...
int main()
{
    while (scanf("%d", &n) == 1)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            {
                if ((i == 1 && j == 1) || (i == n && j == n))
                    ans.add_edge((i - 1) * n + j , (i - 1) * n + j + n * n, 2, - Map[i][j]);
                else
                    ans.add_edge((i - 1) * n + j , (i - 1) * n + j + n * n , 1, -Map[i][j]);
                for (int k = 0; k < 2; k++)
                {
                    int tx = i + xdir[k];
                    int ty = j + ydir[k];
                    if (ok(tx, ty))
                    {
                        ans.add_edge((i - 1) * n + j + n * n, (tx - 1) * n + ty, 1, 0);
                    }
                }
            }
        }
        printf("%d\n", -ans.MincotMaxflow(1, n * n * 2) - Map[1][1] - Map[n][n]);
    }
}