...
int main()
{
	...
	while(T--)
	{
		in(x1),in(y1),in(x2),in(y2),in(x3),in(y3),in(x),in(y);
		...
		TLargeFloat c(X1-X2);
		TLargeFloat d(Y1-Y2);
		TLargeFloat e((X1*X1-X3*X3-Y3*Y3+Y1*Y1)/2.0);
		TLargeFloat f((X1*X1-X2*X2-Y2*Y2+Y1*Y1)/2.0);
		TLargeFloat X0((b*f-d*e)/(b*c-a*d));
		TLargeFloat Y0((c*e-a*f)/(b*c-a*d));
		TLargeFloat temp((X1-X0)*(X1-X0)+(Y1-Y0)*(Y1-Y0));
		if((X-X0)*(X-X0)+(Y-Y0)*(Y-Y0)>temp)
			cout << "Accepted" << endl;
		else
			cout << "Rejected" << endl;
	}
