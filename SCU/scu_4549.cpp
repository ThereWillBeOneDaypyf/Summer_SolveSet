...
void push_down(int step)
{
	...
	t[step*2].sum += 1LL * (t[step*2].r - t[step*2].l +1) * t[step].vis;
	t[step*2+1].sum += 1LL * (t[step*2+1].r - t[step*2+1].l + 1) * t[step].vis;
	t[step].vis = 0;
}
...
void update(int l,int r,long long val,int step)
{
	if(t[step].l == l && t[step].r == r)
	{
		t[step].sum += 1LL * (t[step].r - t[step].l + 1) * val;
		t[step].vis += val;	
		return;
	}
